...
var indexCollection = function(db) {
    return co(function*() {
        const results = yield db.collection('restaurants').createIndex({ "name": 1 }, null);
        console.log(results);
        return results;
    });
};
...
var aggregateDocuments = function(db, callback) {
    return co(function*() {
        const results = yield db.collection('restaurants').aggregate([{ '$match': { "categories": "Bakery" } }, { '$group': { '_id': "$stars", 'count': { '$sum': 1 } } }]).toArray();
        console.log(results)
        return results;
    });
}
